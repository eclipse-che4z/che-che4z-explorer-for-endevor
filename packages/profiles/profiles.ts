...
const initAndGetProfileManager = (logger: Logger) => (
  profileRootDirectory: string
) => async (
  profileType: ProfileTypes | string
): Promise<CliProfileManager | undefined> => {
  let profileManagerOrError = getProfileManagerFromDir(logger)(
  ...
  if (!profileManagerOrError) {
    ...
    try {
      ...
      profileManagerOrError = getProfileManagerFromDir(logger)(
        profileRootDirectory
      )(profileType);
    } catch (error) {
      logger.trace(
...
export const getServiceProfileByName = (logger: Logger) => async (
  ...
  const profileManager = await getProfileManager(logger)(ProfileTypes.ENDEVOR);
  if (profileManager) {
    let serviceProfile;
    try {
