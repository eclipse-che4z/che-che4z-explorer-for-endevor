...
import { askForCompareWithChangeLevel } from '../dialogs/compareWithDialog';

export const compareWithCommand = async (
  logger: Logger,
  ...
  const historyData = getHistoryData(changeLvlUri);
  const pickedVvll = await askForCompareWithChangeLevel(
    changeNode.vvll,
    historyData
  );
  refreshElementHistoryTree(
    changeLvlUri,
    HistoryViewModes.SHOW_IN_EDITOR,
    pickedVvll
  );
};
