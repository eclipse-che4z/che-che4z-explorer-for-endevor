...
export async function proxyRetrieveAcmComponents(
    ...
    const endevorElement = endevorQualifierToElement(
        endevorQualifier,
        instance
    );
    ...
    if (listSubsystemResponse.returnCode > 0) {
        logger.error(
            listSubsystemResponse.messages
                ? listSubsystemResponse.messages[1] ??
                      listSubsystemResponse.messages[0]
    ...
    if (listSystemResponse.returnCode > 0) {
        logger.error(
            listSystemResponse.messages
                ? listSystemResponse.messages[1] ??
                      listSystemResponse.messages[0]
                : 'Error retrieving subsystem',
